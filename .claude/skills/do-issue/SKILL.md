...
- **Concern**: 軽微なら修正、判断に迷うならそのまま PR に注記。修正しなかった Concern は `concerns` リストに記録し、ステップ 7・8 で使用する
- **Pass**: 次ステップへ
...
## Concerns
<!-- セルフレビューで見つかった未修正の懸念点。なければセクションごと省略 -->
- ...
EOF
)"
```
...
- セルフレビューで未修正の Concern がある場合、`## Concerns` セクションに記載する（なければ省略）
...
- 懸念点がある場合は必ず完了報告に含める。ユーザーがマージ前に判断できるようにする
- 懸念点がない場合は「懸念点」セクションを省略する
