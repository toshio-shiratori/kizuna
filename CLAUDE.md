...
- Always run `pnpm lint`, `pnpm format:check`, and `pnpm knip` before declaring work complete
...
- **NEVER commit with warnings or errors.** All checks (`pnpm tsc --noEmit`,
  `pnpm test`, `pnpm lint`, `pnpm format:check`, `pnpm knip`) must pass with
...
3. **実装** — コーディングとテスト
4. **検証** — `pnpm tsc --noEmit && pnpm test && pnpm lint && pnpm format:check && pnpm knip`
