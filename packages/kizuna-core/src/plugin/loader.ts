...
import type { Plugin, PluginConfig, Logger } from "../index.js";
...
export interface LoadPluginManagerOptions {
  logger?: Logger;
...
export async function loadPluginManager(
  ...
  cwd: string,
  options?: LoadPluginManagerOptions,
  ...
  for (const [packageName, pluginConf] of enabledPlugins) {
    try {
      ...
      if (!plugin) {
        logger.warn(`No plugin export found in "${packageName}"`);
        continue;
      }

      if (options?.filterPlugin && !options.filterPlugin(plugin)) {
        continue;
      }
